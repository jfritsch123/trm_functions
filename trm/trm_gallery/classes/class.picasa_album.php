 ...
 * Class picasaGallery
 */
class PicasaAlbum extends Gallery
{
	/*
	...
	protected function itemList(){
		...
		foreach ($items as $item) {
			...
			$x['date'] = date('d.m.Y',strtotime(getNodeValue($item,'pubDate')));
			...
			$linkUrl = getNodeValue($item,'guid');
			$linkUrl = substr($linkUrl,0,strpos($linkUrl,'?'));
			$linkUrl = str_replace('entry/base','feed/api',$linkUrl);
			$linkUrl .= '?alt=rss';
			$x['linkUrl'] = $linkUrl;
